...
void get_filename_from_path(char* path, char ** filename)
{
    ...
    //Start filename as a whole path
    *filename = path;
...
//Get this function here: https://github.com/gcc-mirror/gcc/blob/master/libiberty/crc32.c
...
#define BUFFER_CRC32_CALC 512
bool file_crc32(uint32_t* crc, const char* path)
{
    FILE* p_file = NULL;
    bool ret = false;
    uint8_t buffer[BUFFER_CRC32_CALC] = {0};
    size_t size_buffer_send = BUFFER_CRC32_CALC;
    size_t file_size = 0;
    size_t file_size_acum = 0;

    //Sanity
    if(path == NULL || crc == NULL)
        goto out;
    ...
    *crc = 0;
    ...
    //Get CRC by blocks
    while(size_buffer_send == BUFFER_CRC32_CALC){
        //Set the pointer to rigth position
        if(fseek(p_file, file_size_acum, SEEK_SET) == -1)
            goto out;
        //Read the buffer to calculate
        size_buffer_send = fread(buffer, 1, size_buffer_send, p_file);
        //Calculate
        *crc = xcrc32 ((const unsigned char *)buffer, (int)size_buffer_send, *crc);
        //Set to the acumulator the size
        file_size_acum += size_buffer_send;
    }

    //Verify if all file was calculate
    if(file_size != file_size_acum)
        goto out;
...
bool is_slash_end_str(const char* p_str)
{
    ...
    return (p_str[size-1] == '\\' || p_str[size-1] == '/' ? true : false);
}
